    ...
    public int delete(CommandContext<CommandSourceStack> context) {
        ...
        plugin.getPlatform().executeAsync(() -> {
            try {
                ...
                ForgeMessageUtil.sendMessage(context.getSource(), Component.literal(ForgeMessageUtil.format("coupon_deleted")).setStyle(CmdUtil.SUCCESS_STYLE));
            } catch (Exception e) {
                ForgeMessageUtil.sendMessage(context.getSource(), Component.literal(e.getMessage()).setStyle(CmdUtil.ERROR_STYLE));
            }
        });
