...
public class CouponCmd {
    ...
    public int create(CommandContext<CommandSource> context) {
        ...
        try {
            ...
            ForgeMessageUtil.sendMessage(context.getSource(), new StringTextComponent(ForgeMessageUtil.format("coupon_creation_arg_parse_failure", e.getMessage()))
                    .setStyle(CmdUtil.ERROR_STYLE));
            return 0;
        }
        ...
        plugin.getPlatform().executeAsync(() -> {
            try {
                ...
                ForgeMessageUtil.sendMessage(context.getSource(), new StringTextComponent(ForgeMessageUtil.format("coupon_creation_success", coupon.getCode()))
    ...
    public int delete(CommandContext<CommandSource> context) {
        ...
        plugin.getPlatform().executeAsync(() -> {
            try {
                ...
                ForgeMessageUtil.sendMessage(context.getSource(), new StringTextComponent(ForgeMessageUtil.format("coupon_deleted")).setStyle(CmdUtil.SUCCESS_STYLE));
            } catch (Exception e) {
                ForgeMessageUtil.sendMessage(context.getSource(), new StringTextComponent(e.getMessage()).setStyle(CmdUtil.ERROR_STYLE));
            }
        });
