...
public class BuyCommand implements Command<CommandSourceStack> {
    ...
    public int run(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        ForgeMessageUtil.sendMessage(context.getSource(), Component.literal(ForgeMessageUtil.format("To view the webstore, click this link: ")).withStyle(style -> style.withColor(ChatFormatting.GREEN)
                .withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, plugin.getServerInformation().getAccount().getDomain()))));
        ForgeMessageUtil.sendMessage(context.getSource(), Component.literal(plugin.getServerInformation().getAccount().getDomain()).withStyle().withStyle(style -> style.withColor(ChatFormatting.BLUE).withUnderlined(true)
                .withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, plugin.getServerInformation().getAccount().getDomain()))));
        ForgeMessageUtil.sendMessage(context.getSource(), Component.literal("                                            ").withStyle(ChatFormatting.STRIKETHROUGH));
        return 1;
    }
